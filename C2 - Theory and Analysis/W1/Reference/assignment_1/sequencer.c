// Sergey Stain, course 2, assignment 1 sequencer for RM service set
// Non-Harmonic, Below LUB, Feasible, Margin (Safe) 
//
...
//***** CONFIG *****************************************************************
//
static const char* kSyslogPrefixStr = "[COURSE:2][ASSIGNMENT:1]";
...
static const bool kStopAtLCM = true;
...
static const struct {
    ...
    {service1,  2, -1, kServicesCPU, 1}, 
    {service2, 10, -2, kServicesCPU, 1}, 
    {service3, 15, -3, kServicesCPU, 2},
};
// LCM of all periods should be calced: and used as a wraparound time point
static const int kLCMPeriod = 30; // sequence will repeat each kLCMPeriod ticks
//
//******************************************************************************
...
int main (int argc, char* argv[]) {
    ...
    const struct itimerspec itime = {{0,kSequencerTickMilliSec*1000000}, 
        {0,kSequencerTickMilliSec*1000000}};
    timer_settime(timer, 0, &itime, NULL);

    // Wait for one period for the timer can proc once
    nanosleep(&itime.it_interval, NULL); // can wakes erlier, but it's OK
