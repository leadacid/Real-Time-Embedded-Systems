...
void main(void)
{
   ...
   uname(&unameData);
   syslog(LOG_INFO, "[COURSE:2][ASSIGNMENT:1] %s %s %s %s %s GNU/Linux\n", 
	        unameData.sysname,
		unameData.nodename, 
	        unameData.release,
	     	unameData.version,
...
void *Sequencer(void *threadp)
{
   ...
   // syslog(LOG_CRIT, "RTSEQ: start on cpu=%d @ sec=%lf after %lf with dt=%lf\n", sched_getcpu(), current_time, last_time, delta_t);
    ...
    } while(!abortTest && (seqCnt < 29));//threadParams->sequencePeriods));
...
void *Service_1(void *threadp)
{
    ...
    while(!abortS1)
    {
        ...
        current_time=getTimeMsec();
        syslog(LOG_INFO, "[COURSE:2][ASSIGNMENT:1]: Thread 1 start %llu @ sec=%lf\n on core=%d", S1Cnt, current_time, sched_getcpu());
    }
...
void *Service_2(void *threadp)
{
    ...
    while(!abortS2)
    {
       ...
       // syslog(LOG_CRIT, "S2: release %llu @ sec=%lf\n", S2Cnt, current_time);
        syslog(LOG_INFO, "[COURSE:2][ASSIGNMENT:1]: Thread 2 start %llu @ sec=%lf\n on core=%d", S2Cnt, current_time, sched_getcpu());
    }
...
void *Service_3(void *threadp)
{
    ...
    while(!abortS3)
    {
        ...
        syslog(LOG_INFO, "[COURSE:2][ASSIGNMENT:1]: Thread 3 start %llu @ sec=%lf\n on core=%d", S3Cnt, current_time, sched_getcpu());
