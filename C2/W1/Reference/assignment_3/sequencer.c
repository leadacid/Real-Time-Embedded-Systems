...
//***** CONFIG *****************************************************************
//
static const char* kSyslogPrefixStr = "[COURSE:2][ASSIGNMENT:3]";
...
static const struct {
    ...
    {service3, 10, -3, kServicesCPU, 2},
    {service4, 20, -4, kServicesCPU, 1.95},
};
// LCM of all periods should be calced: and used as a wraparound time point
static const int kLCMPeriod = 20; // sequence will repeat each kLCMPeriod ticks
//
//******************************************************************************
...
void service4(void) {
    ...
    ++cnt;
    const struct timespec start = rt_clock();
    syntheticLoad(services_params[3].capacity);
    syslog(LOG_DEBUG, "Thread 4 start %4ld @ %ld.%09ld on core %d",  
            cnt, start.tv_sec, start.tv_nsec, sched_getcpu());
}
