...
int delta_t(struct timespec *stop, struct timespec *start, struct timespec *delta_t)
{
  ...
  if(dt_sec == 0)
  {
	  ...
	  {
	         syslog(LOG_INFO, "stop is earlier than start\n");
		 return(ERROR);  
	  }
  }
...
void end_delay_test(void)
{
    double real_dt;
#if 0
  syslog(LOG_INFO, "MY_CLOCK start seconds = %ld, nanoseconds = %ld\n", 
         rtclk_start_time.tv_sec, rtclk_start_time.tv_nsec);
  
  syslog(LOG_INFO,"MY_CLOCK clock stop seconds = %ld, nanoseconds = %ld\n", 
  ...
  printf("MY_CLOCK clock DT seconds = %ld, msec=%ld, usec=%ld, nsec=%ld, sec=%6.9lf\n", 
         rtclk_dt.tv_sec, rtclk_dt.tv_nsec/1000000, rtclk_dt.tv_nsec/1000, rtclk_dt.tv_nsec, real_dt);
#if 0
  syslog(LOG_INFO, "Requested sleep seconds = %ld, nanoseconds = %ld\n", 
         sleep_requested.tv_sec, sleep_requested.tv_nsec);

  syslog(LOG_INFO, "\n");
  syslog(LOG_INFO, "Sleep loop count = %ld\n", sleep_count);
#endif
...
int main(void)
{
   ...
   if (rc)
   {
       syslog(LOG_INFO, "ERROR; sched_setscheduler rc is %d\n", rc);
   ...
   if (rc)
   {
       ...
       syslog(LOG_INFO, "ERROR; pthread_create() rc is %d\n", rc);
